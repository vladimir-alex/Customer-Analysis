...
df_response <- df[, c(13,3, 4, 5, 8, 9, 16:20, 26, 30,32)]
...
#linear regression: MntFishProducts
linear_fit <- lm(MntFishProducts~., data = df_response)
summary(linear_fit)
#Adjusted R-squared: 0.4117 

#Prediction
ls_fit <- lm(MntFishProducts~., data = df_response[index, ])
summary(ls_fit)
#Adjusted R-squared:  0.4292  

predict(ls_fit, df_response[-index, ])
cor(predict(ls_fit, df_response[-index, ]), 
    df_response[-index, ]$MntFishProducts, use = "complete.obs")
#correlation: 0.6246064


#Ridge regression: MntFishProducts
ridge_reg_x <- model.matrix(MntFishProducts~., data = df_response)[, -1] 
ridge_reg_y <- df_response$MntFishProducts
...
rf_fit <- randomForest(formula = MntFishProducts ~.,
                       data = df_response, 
                       mtry = (ncol(df_response[index, ]) - 1)/ 3, 
                       ntree = 1000, importance = TRUE)

rf_pred <- predict(rf_fit, df_response[-index, ])
mean((df_response[-index, ]$MntFishProducts - rf_pred)^2)
#[1] 274.9406

importance(rf_fit)
varImpPlot(rf_fit, 
           main = "Random Forest Variables Importance for Amount Spent on Fish Products")
...
# Fit regression tree
reg_tree <- tree(MntFishProducts ~., df_response[index, ])
summary(reg_tree)

# MSE in test data
mean((df_response[-index, ]$MntFishProducts - predict(reg_tree, df_response[-index, ]))^2)
#mse = 1818.463
...
# MSE in test data
mean((df_response[-index, ]$MntFishProducts - predict(prune_reg_tree, df_response[-index, ]))^2)
#MSE = 1813.151
