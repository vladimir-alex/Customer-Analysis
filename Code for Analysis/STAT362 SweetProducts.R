...
df_response <- df[, c(14,3, 4, 5, 8, 9, 16:20, 26, 30,32)]
...
#linear regression: MntSweetProducts
linear_fit <- lm(MntSweetProducts~., data = df_response)
summary(linear_fit)
#Adjusted R-squared: 0.3824 

##Prediction
ls_fit <- lm(MntSweetProducts~., data = df_response[index, ])
summary(ls_fit)
#Adjusted R-squared:  0.4133 

predict(ls_fit, df_response[-index, ])
cor(predict(ls_fit, df_response[-index, ]), 
    df_response[-index, ]$MntSweetProducts, use = "complete.obs")
#correlation: 0.5804429


#Ridge regression: MntSweetProducts
ridge_reg_x <- model.matrix(MntSweetProducts~., data = df_response)[, -1] 
ridge_reg_y <- df_response$MntSweetProducts
...
rf_fit <- randomForest(formula = MntSweetProducts ~.,
                       data = df_response, 
                       mtry = (ncol(df_response[index, ]) - 1)/ 3, 
                       ntree = 1000, importance = TRUE)

rf_pred <- predict(rf_fit, df_response[-index, ])
mean((df_response[-index, ]$MntSweetProducts - rf_pred)^2)
#[1] 181.0362

importance(rf_fit)
varImpPlot(rf_fit,
           main = "Random Forest Variables Importance for Amount Spent on Sweet Products")
...
# Fit regression tree
reg_tree <- tree(MntSweetProducts ~., df_response[index, ])
summary(reg_tree)

# MSE in test data
mean((df_response[-index, ]$MntSweetProducts - predict(reg_tree, df_response[-index, ]))^2)
#mse = 1126.875
...
# MSE in test data
mean((df_response[-index, ]$MntSweetProducts - predict(prune_reg_tree, df_response[-index, ]))^2)
#MSE = 1129.661
